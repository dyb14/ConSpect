...
from conspect.dataset.load_data import LoadData
from conspect.dataset.stmatrix import STMatrix
...
def get_dataset(
    ...
    va_X = datasets["X_valid"]
    va_y = datasets["y_valid"]
...
class MakeDataset(LoadData):
    def create_dataset(
        ...
        closeness = main_data["closeness"]
        period = main_data["period"]
        trend = main_data["trend"]
        y = main_data["y"]

        train_closeness = closeness[:-len_test]
        train_period = period[:-len_test]
        train_trend = trend[:-len_test]
        train_y = y[:-len_test]

        valid_closeness = closeness[-len_test:]
        valid_period = period[-len_test:]
        valid_trend = trend[-len_test:]
        valid_y = y[-len_test:]

        train_X = [train_closeness, train_period, train_trend]
        valid_X = [valid_closeness, valid_period, valid_trend]
        ...
        if any([use_meta, use_holiday, use_meteorol]):
            ...
            valid_meta = meta_features[-len_test:]
            valid_X.append(valid_meta)

        timestamps_y = main_data["timestamps_y"]
        train_timestamps = timestamps_y[:-len_test]
        valid_timestamps = timestamps_y[-len_test:]
    ...
    def _create_main_data(
        ...
        for data, timestamps in zip(data_all_scaled, timestamp_all):
            ...
            closeness.append(d_mtx["closeness"])
            period.append(d_mtx["period"])
            trend.append(d_mtx["trend"])
            y.append(d_mtx["y"])
            ts_y.append(d_mtx["timestamps_y"])

        main_data = {
            "closeness": np.vstack(closeness),
